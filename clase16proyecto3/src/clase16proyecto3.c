...
int resizeArrayError(int* pArray, int* sizeActual,int sizeNuevo)
{
	...
	if(pArray != NULL && sizeActual != NULL && sizeNuevo >= 0)
	{
		...
		if(pArrayTemp != NULL)
		{
			*sizeActual = sizeNuevo; // variable local, por escribir acá no estamos afectando a la VAR del Main
			pArray = NULL;
			pArrayTemp = NULL;
			retorno = 0;
		}
...
int main(void)
{
	...
	if(pArrayEdades != NULL)
	{
		...
		if(!resizeArray(&pArrayEdades,&sizeArrayEdades,25)) // &: le paso la dirección de memo del puntero que aloja la dirección de memo del Array original, y la modifico
		{
			printf("RESIZE OK\n");
			imprimirArray(pArrayEdades,sizeArrayEdades);
		}
